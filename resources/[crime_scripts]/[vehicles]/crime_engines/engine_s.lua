...
function calculateVehicleEngine(vehicle)
	...
	local type = getElementData(vehicle, "vehicleSound:type")
...
function addVehicleEngine(vehicle)
	...
	local model = getElementModel(vehicle)
	if data then 
		if getElementData(vehicle, "vehicle:us1") == 1 then us3montage = true else us3montage = false end
		if getElementData(vehicle, "vehicle:us2") == 1 then turbomontage = true else turbomontage = false end
		setElementData(vehicle, "vehicle:upgrades", {turbo=turbomontage, als=us3montage})
		...
		if engine then 
			...
			setElementData(vehicle, "vehicleSound:engine", engine)
...
function onResourceStart()
	for k, v in ipairs(getElementsByType("vehicle")) do 
		local type = getElementData(v, "vehicleSound:type")
		if not type then
			type = getVehicleTypeByModel(getElementModel(v))
			setElementData(v, "vehicleSound:type", type)
		end 
...
function onVehicleEnter(player, seat, jacked)
	if seat == 0 then 
		local type = getElementData(source, "vehicleSound:type")
		if not type then
			type = getVehicleTypeByModel(getElementModel(source))
			setElementData(source, "vehicleSound:type", type)
		end 
