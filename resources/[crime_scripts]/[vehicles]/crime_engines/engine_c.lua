...
function updateEngines(dt)
	...
	for vehicle, data in pairs(streamedVehicles) do 
		if isElement(vehicle) then
			local engine = getElementData(vehicle, "vehicleSound:engine")
			if engine then
...
function streamInVehicle(vehicle)
	if not streamedVehicles[vehicle] then
		if isElement(vehicle) and getElementData(vehicle, "vehicleSound:engine") then 
            ...
            if (getVehicleEngineState(vehicle) == true) then
                if(getVehicleCurrentGear(vehicle) > 0) then
                    ...
                    if (vehicleRPM < 650) then
                        vehicleRPM = math.random(150, 750)
