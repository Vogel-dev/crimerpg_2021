...
addEvent("anims:startanim", true)
addEventHandler("anims:startanim", root, function(name,name2)
    if (isPedInVehicle(source)) then return end
    if isPedWearingJetpack(source) == true then return end
    if getElementData(source,"player:pracadorywczna") then return end
    bindKey(source, "ENTER", "down", stopAnimacja)
    for i,v in pairs(anims) do 
        if v[2] == name then 
            if v[3] == name2 then 
                setPedAnimation ( source, v[2], v[3], v[4], v[5], v[6] )
            end
        end
    end
end)

addEvent("anims:stopanim", true)
addEventHandler("anims:stopanim", root, function()
...
for i,v in pairs(anims) do 
	addCommandHandler(v[1],function(plr,cmd)
        ...
        setTimer(function()
            if getElementData(plr,"player:pracadorywczna") then return end
            bindKey(plr, "ENTER", "down", stopAnimacja)
            setPedAnimation ( plr, v[2], v[3], v[4], v[5], v[6] )
        end, 100, 1)
	end)
end