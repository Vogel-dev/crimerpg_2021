...
function DeveloperMode()
    if getElementData(localPlayer,"player:admin") == false then 
    	--exports.srnoti:create_alert('error','Nie posiadasz uprawnień!!')
    	return
    end
...
function putPlayerInPosition(timeslice)
	...
	--if getKeyState("0") then timeslice = timeslice*10 end
...
function toggleAirBrakec()
	...
	if type(levele) == "number" then
        if levele < 2 then
            exports.crime_gui:addNoti("Nie posiadasz uprawnień.", "error")
    	return end
...
bindKey("1","down",toggleAirBrakec)
