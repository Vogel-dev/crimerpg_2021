...
local zoom = math.max(1, math.min(2, 1920 / Display.Width))
...
addEventHandler('onClientRender', root,
	function()
		if getElementData(localPlayer, "player:hide_minimap") or getElementData( localPlayer, "player:always_hide_minimap" ) then return end
		if (not Minimap.IsVisible and Bigmap.IsVisible) then
					...
					if getElementDimension(blip) == getElementDimension(localPlayer) then
						if (localPlayer ~= getElementAttachedTo(blip)) then
							local blipSettings = {
								...
								['size'] = getElementData(blip, 'blipSize') or 28,
		...
		elseif (Minimap.IsVisible and not Bigmap.IsVisible) then
			if (radarSettings['showStats']) then
				Minimap.PosY = ((Display.Height - 30) - Stats.Bar.Height) - Minimap.Height;
			else
				Minimap.PosY = (Display.Height - 20) - Minimap.Height;
			end
			...
			if (getElementInterior(localPlayer) == 0) then
				...
				for _, blip in ipairs(serverBlips) do
					...
					if (localPlayer ~= getElementAttachedTo(blip) and getElementInterior(localPlayer) == getElementInterior(blip) and getElementDimension(localPlayer) == getElementDimension(blip)) then
						...
						local blipSettings = {
							['color'] = {255, 255, 255, 255},
							['size'] = getElementData(blip, 'blipSize') or 32,
							['exclusive'] = getBlipIcon(blip) == 41 and true or false,
							['icon'] = getBlipIcon(blip)
						};
				...
				dxDrawText('Utracono sygnał GPS...', Minimap.PosX, Minimap.PosY + 20, Minimap.PosX + Minimap.Width, Minimap.PosY + 20 + Minimap.Height, tocolor(255, 255, 255, 255 * math.abs(getTickCount() % 2000 - 1000) / 1000), 0.40, Fonts.Roboto, 'center', 'center', false, false, false, true, true);
