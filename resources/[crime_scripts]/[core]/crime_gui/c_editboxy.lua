...
local sxts, syts = guiGetScreenSize()
zmienna = 1920/sxts

function testsadcyce()
	for i,v in pairs(dxEditBox) do 
		if dxEditBox[i].aktywny then 
...
setTimer(testsadcyce,10000,0)

function createEditbox( id, xS, yS, w, h, widoczny, tekst, zamaskowany, dlugosc, czcionka, spacja, typ, kolor, prostokat, kolorProstokat, kolorKlik, skala, clip, ms, tekstBool, tekstPomocniczy, tekstPomocniczyKolor, maZniknac, skalaPomoc, czcionkaPomoc, postGUI, wstawka, kolorWstawki, specjalneZnaki, mozeKliknac)
	if dxEditBox[id] == nil then
        ...
        dxEditBox[id].kolorKlik = kolorKlik
...
function changeEditboxText ( id, text )
	if dxEditBox[id] ~= nil then
...
function changeEditboxVisibility( id, bool )
	if dxEditBox[id] ~= nil then
...
function setEditboxSize( id, w, h )
	if dxEditBox[id] ~= nil then
		dxEditBox[id].w = w
		dxEditBox[id].h = h
	end
	...
	for k, v in pairs( dxEditBox ) do
		if dxEditBox[k].widoczny then
            if dxEditBox[k].prostokat then
				...
				dxDrawRectangle(dxEditBox[k].x, dxEditBox[k].y+(40/zmienna), dxEditBox[k].w, 4/zmienna, tocolor(dxEditBox[k].kolorKlik[1], dxEditBox[k].kolorKlik[2], dxEditBox[k].kolorKlik[3], dxEditBox[k].kolorKlik[4]), dxEditBox[k].postGUI)
                else 
                dxDrawRectangle(dxEditBox[k].x, dxEditBox[k].y+(40/zmienna), dxEditBox[k].w, 4/zmienna, tocolor(dxEditBox[k].kolorProstokat[1], dxEditBox[k].kolorProstokat[2], dxEditBox[k].kolorProstokat[3], dxEditBox[k].kolorProstokat[4]), dxEditBox[k].postGUI)
                end
            end

			if dxEditBox[k].tekstBool then
				dxDrawText( dxEditBox[k].tekstPomocniczy, dxEditBox[k].x, dxEditBox[k].y, dxEditBox[k].x + dxEditBox[k].w, dxEditBox[k].y + dxEditBox[k].h, tocolor(dxEditBox[k].tekstPomocniczyKolor[1], dxEditBox[k].tekstPomocniczyKolor[2], dxEditBox[k].tekstPomocniczyKolor[3], dxEditBox[k].tekstPomocniczyKolor[4]), 0.7, dxEditBox[k].czcionkaPomoc, "center", "center", dxEditBox[k].clip, nil, dxEditBox[k].postGUI)
			end
			if dxEditBox[k].zamaskowany == false then
				dxDrawText( dxEditBox[k].tekst, dxEditBox[k].x , dxEditBox[k].y, dxEditBox[k].x + dxEditBox[k].w, dxEditBox[k].y + dxEditBox[k].h, tocolor(dxEditBox[k].kolor[1], dxEditBox[k].kolor[2], dxEditBox[k].kolor[3], dxEditBox[k].kolor[4]), 0.7, dxEditBox[k].czcionka, "center", "center", dxEditBox[k].clip, nil, dxEditBox[k].postGUI)
				else
				dxDrawText( dxEditBox[k].tekst:gsub(".","•"), dxEditBox[k].x , dxEditBox[k].y + dxEditBox[k].h/3, dxEditBox[k].x + dxEditBox[k].w, dxEditBox[k].y + dxEditBox[k].h, tocolor(dxEditBox[k].kolor[1], dxEditBox[k].kolor[2], dxEditBox[k].kolor[3], dxEditBox[k].kolor[4]), 0.7, dxEditBox[k].czcionka, "center", "center", dxEditBox[k].clip, nil, dxEditBox[k].postGUI)
			end
			if dxEditBox[k].aktywny then
				if dxEditBox[k].wstawka then
				sekundy = getTickCount() / 400
					if dxEditBox[k].dlugoscDX < (dxEditBox[k].x +dxEditBox[k].w-0.004 ) then
						if dxEditBox[k].start then
						dxDrawRectangle(math.floor(dxEditBox[k].x + dxEditBox[k].w/2), dxEditBox[k].y+0.005,  0.002 ,  dxEditBox[k].h * 0.85 , tocolor( dxEditBox[k].kolorWstawki[1], dxEditBox[k].kolorWstawki[2], dxEditBox[k].kolorWstawki[3], math.abs(math.sin(sekundy) * 220)), dxEditBox[k].postGUI)
						else
						dxDrawRectangle(math.floor(dxEditBox[k].dlugoscDX), dxEditBox[k].y+0.005, 0.002 ,  dxEditBox[k].h * 0.85 , tocolor( dxEditBox[k].kolorWstawki[1], dxEditBox[k].kolorWstawki[2], dxEditBox[k].kolorWstawki[3], math.abs(math.sin(sekundy) * 220)), dxEditBox[k].postGUI)
						end
					end
				end
			end
...
if getKeyState("mouse1") then
	if math.floor(dxEditBox[k].dlugoscTextuDX) >= math.floor(dxEditBox[k].dlugoscDX) + ( xck - math.floor(dxEditBox[k].dlugoscDX)) then
		...
		dxDrawRectangle ( math.floor(dxEditBox[k].dlugoscDX), dxEditBox[k].y+0.005,  xck - math.floor(dxEditBox[k].dlugoscDX) ,  dxEditBox[k].h * 0.85 , tocolor( 55, 254, 233, 255))
	end
end
...
function zwrocWartoscPrzesuniecia ( k, bool )
	if bool ~= true then
	local maxCarret = math.floor(dxGetTextWidth( dxEditBox[k].tekst, dxEditBox[k].skala, dxEditBox[k].czcionka )/1280)
	local pozycjaTekstuDX = math.floor ( (dxEditBox[k].x + dxEditBox[k].w/2 - (maxCarret/2)/1280 )+0.001  )
	dxEditBox[k].dlugoscTextuDX = pozycjaTekstuDX
	dxEditBox[k].maxTextDX = maxCarret
	return  math.abs(math.floor((dxGetTextWidth( dxEditBox[k].tekst, dxEditBox[k].skala, dxEditBox[k].czcionka )/1280)+pozycjaTekstuDX))
		else
		local maxCarret = math.floor(dxGetTextWidth( (dxEditBox[k].tekst:gsub(".","•")), dxEditBox[k].skala, dxEditBox[k].czcionka )/1280)
		local pozycjaTekstuDX = math.floor (  (dxEditBox[k].x + dxEditBox[k].w/2 - (maxCarret/2)/1280 )+0.001  )
		dxEditBox[k].dlugoscTextuDX = pozycjaTekstuDX
		dxEditBox[k].maxTextDX = maxCarret
		return  math.abs(math.floor((dxGetTextWidth( dxEditBox[k].tekst:gsub(".","•"), dxEditBox[k].skala, dxEditBox[k].czcionka )/1280)+pozycjaTekstuDX))
	end
end
...
function zwrocWartoscPrzesunieciaAktywnego ( k, bool )
	...
	dxEditBox[k].dlugoscTextuDX = pozycjaTekstuDX
	dxEditBox[k].maxTextDX = maxCarret
	return  math.abs(math.floor((dxGetTextWidth( (dxEditBox[k].tekst:sub( 1, dxEditBox[k].pozycjaCarretu)), dxEditBox[k].skala, dxEditBox[k].czcionka )/1280)+pozycjaTekstuDX))
		else
		local maxCarret = math.floor(dxGetTextWidth(( dxEditBox[k].tekst:gsub(".","•")), dxEditBox[k].skala, dxEditBox[k].czcionka )/1280)
		local pozycjaTekstuDX = math.floor (  (dxEditBox[k].x + dxEditBox[k].w/2 - (maxCarret/2)/1280 )+0.001  )
		dxEditBox[k].dlugoscTextuDX = pozycjaTekstuDX
		dxEditBox[k].maxTextDX = maxCarret
		return  math.abs(math.floor((dxGetTextWidth( ((dxEditBox[k].tekst:sub( 1, dxEditBox[k].pozycjaCarretu)):gsub(".","•")), dxEditBox[k].skala, dxEditBox[k].czcionka )/1280)+pozycjaTekstuDX))
	end
end
	...
	if przycisk == "left" and stan == "down" then
		...
		if dxEditBox[k].tekstPomocniczyKopia ~= nil then
			...
			if dxEditBox[k].widoczny then
				...
				if sprawdzObszar( dxEditBox[k].x, dxEditBox[k].y, dxEditBox[k].w, dxEditBox[k].h ) then
					if dxEditBox[k].aktywny == false then
						...
						if dxEditBox[k].zamaskowany then
							...
							pozycjaTekstu = math.floor (  (dxEditBox[k].x + dxEditBox[k].w/2 - (maxCarret/2)/1280 )  )
						else
							maxCarret = math.floor(dxGetTextWidth( dxEditBox[k].tekst:sub(1, #dxEditBox[k].tekst), dxEditBox[k].skala, dxEditBox[k].czcionka )/1280)
							pozycjaTekstu = math.floor ( (dxEditBox[k].x + dxEditBox[k].w/2 - (maxCarret/2)/1280 )  )
						end
						...
						dxEditBox[k].carretPrzesuniety = false
							if dxEditBox[k].zamaskowany then
								pozycjaCarretu = math.abs(math.floor((dxGetTextWidth( (dxEditBox[k].tekst:sub(1, i)):gsub(".","•"), dxEditBox[k].skala, dxEditBox[k].czcionka )/1280)+pozycjaTekstu))
								else
									pozycjaCarretu = math.abs(math.floor((dxGetTextWidth( dxEditBox[k].tekst:sub(1, i), dxEditBox[k].skala, dxEditBox[k].czcionka )/1280)+pozycjaTekstu))
								end
